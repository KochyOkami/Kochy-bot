    ...
    async execute(interaction) {
        ...
        const text = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '/unlink `id` :', value: 'Unlink channel where the command is currently send, Optionnal `channel`if you want to unlink\
                                         a specific channel.' },
                { name: '/show_link `*all` :', value: 'Show with witch channel this channel is linked. Option `all`, true for see all links.' },
                { name: "/set `option` `*id` :", value: "Set channel to specifiqe channel. Option for use a other channel. Option: `Yaoi Hard`, `Yaoi Light`."},
                { name: "/say `text` `*user` :", value: "To send a message with the bot or is `user` is passed to send message with the user name/avatar."},
                { name: "/yaoi `type` `title` `author` `description` `language` `link` `*image` :", value: "To share a yaoi, the `type` is for the NSFW\
