...
bot.on("ready", async () => {
    try {
        ...
        //send message with all backup files.
        ...
        //sync the cookies liste to the server for avoiding troubles.
        var interval = setInterval(function () {
        ...
        //send all avatars and names of the cuties users of the server ^^
        var interval = setInterval(function () {
            var cookie_user = JSON.parse(fs.readFileSync('./cookie_user.json', 'utf8'));
            var myJSONObject = { 'users': cookie_user, 'password': settings.server_password };
            //Custom Header pass
            var headersOpt = {
                "content-type": "application/json",
            };
...
bot.on('guildMemberAdd', member => {
    ...
    var id = member.id;
    try {
        //find the waiting role in the server.
        member.guild.roles.fetch(settings.waiting_role)
            .then(role => {
                //add the role to the beauty new members.
                member.roles.add(role, 'A role for avoid some trouble with image >w<')
                    .then(() => {
                        //start the timer for deleting the role.
                    ...
                    message.attachments.forEach(async function (attach) {
                        ...
                        if (accept.indexOf(attach.name.split('.')[-1] != -1)) {
                            ...
                            if (settings.category[message.channel.id]) {
                                ...
                                var myJSONObject = { 'url': attach.url, 'name': name_i.normalize('NFC'), 'category': category, 'author': author_i.normalize('NFC'), 'password': settings.server_password };
                        ...
                        console.log('deleting image...', path, fs.existsSync(path))
